...
import { EC2NativeAppStack, NativeAppConfig } from '../lib/native-stack';
...
configFiles.forEach(configFile => {
  ...
  const config: NativeAppConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));

  const stackId = `${config.appName}Stack`;

  new EC2NativeAppStack(app, stackId, config, { env });
});
